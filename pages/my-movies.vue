...
const filteredMovies = computed(() => {
    ...
    return user.movies.value.filter(movie => movie.title.includes(debouncedSearchTerm.value))
})
</script>
...
<template>
    <div>
        ...
        <UInput size="xl" placeholder="Filter..." v-model="searchTerm" name="search"
        ...
        <div class="flex flex-col items-stretch">
            ...
            <p v-if="debouncedSearchTerm !== '' && user.movies && user.movies.length === 0" class="text-xl">No matching
                results can be found.</p>
            <div class="movie-grid " v-if="user.movies.length > 0">
                <MovieCard :movie="movie" v-for="movie in user.movies" :key="movie.id" />
